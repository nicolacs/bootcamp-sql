...
-- Database: column defintions: VARCHAR(Similar o Java String), INT / INTEGER (Java: int)
-- DECIMAL(13,2) = 11 digits for integer, 2 digits for decimal places
-- boolean (true / false) : database normally we use VACHAR(1) -> Y/N
CREATE TABLE STAFFS (
	ID INTEGER, -- 1,2,3,4...
    ...
    DEPT_CODE VARCHAR(2) -- IT, HR, MK, ETC...
);
...
INSERT INTO STAFFS (ID, STAFF_NAME, GENDER, SALARY, STAFF_EMAIL, DEPT_CODE) 
	VALUES (2, 'PETER LAU', 'M', 31000.0, 'peterlau@hotmail.com', 'HR');
...
SELECT * FROM STAFFS WHERE DEPT_CODE = 'IT' AND (SALARY > 19000 OR GENDER = 'F'); -- OR GO FIRST, THE AND
-- SELECT SPECIFIC COLUMNS
-- SYNTAX: select "column_name1", 'column_name2" .. form "table_name"
...
, S.STAFFS_NAME
, S.ID
, S.EMPLOYMENT_DATE + 33 -- DATE -> INT + OPERATION
FROM STAFFS S;

-- MYSQL (NOT WORKING)
SELECT S.STADD_NAME + '!!!!', CONCAT(S.STAFF_NAME, '!!!!')
FROM STAFFS S;

-- SELECT DETERMAIN WHAT COLUMNS
SELECT S.STAFF_NAME + '!!!!'
, CONCAT(S.STAFF_NAME, '!!!!', '?????')
...
-- FROM VARCHA(20) TO VARCHAR(30) -> EXTEND COLUMN LENGTH
ALTER TABLE STAFFS MODIFY STAFF_NAME VARCHAR(30);

INSERT INTO STAFFS (ID, STAFF_NAME, GENDER, SALARY, DEPT_CODE) 
	VALUES (4, 'vincent LAUABCDEFGHIJKLMNOPQ', 'M', 25000.0, 'MK');
...
UPDATE STAFFS SET STAFF_EMAIL = 'vincentlau@gotmail.com' WHERE ID = 4;
...
-- AS LONG AS YOU HAVE NE AGGREGATION FUNCTION, THE RESULT MUST BE SINGLE ROW
SELECT SUM(SALARY), COUNT(1), COUNT(*), COUNT(ID), MIN(SALARY), MAX(ID), AVG(SALARY) -- , STAFF_NAME, ID //sum係一個row, 黎度2個名, 根本出唔到, 唔可以一齊出
FROM STAFFS
WHERE DEPT_CODE = 'IT';
...
-- STEP 1. EXCLUDE TH EMPLOYEE IN MARKETING
-- STEP 2. GROUP BY
-- STEP 3. SELECT AGG FUNCTIONS
SELECT DEPT_CODE, AVG(SALARY) AS DEPT_AVERAGE_SALARY, COUNT(1) AS NUMBER_OF_DEPT_EMPLOYEE, MAX(SALARY) DEPT_MAX_SALARY
FROM STAFFS
